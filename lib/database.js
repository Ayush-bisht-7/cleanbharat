  ...
  if (!db) {
    ...
    try {
      const adminExists = await db.execute({
        ...
        args: ['admin@cleanbharat.com']
      });
      if (adminExists.rows.length === 0) {
        const passwordHash = await bcrypt.hash("adminpassword", 10);
        await db.execute({
          sql: `INSERT INTO users (name, email, password_hash, role) VALUES (?, ?, ?, ?)`,
          args: ['Admin User', 'admin@cleanbharat.com', passwordHash, 'admin']
        });
      }
    } catch (e) {
...
export async function deleteUser(id) {
  await getDb();
  // Dissociate user posts
  await run('UPDATE posts SET user_id = NULL WHERE user_id = ?', [id]);
  // Delete their likes
  await run('DELETE FROM post_likes WHERE user_id = ?', [id]);
  const stmt = await run('DELETE FROM users WHERE id = ?', [id]);
  return stmt.changes > 0;
}
...
export async function getAdminStats() {
  ...
  const likesCount = await get('SELECT SUM(likes) as count FROM posts');
  return {
